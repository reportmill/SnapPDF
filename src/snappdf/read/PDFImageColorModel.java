...
public class PDFImageColorModel extends ColorModel {
                                                    ...
                                                    boolean hasalpha)
    {
        ...
        if (bpp <= 8) transferType = DataBuffer.TYPE_BYTE;
            ...
            System.err.println("Unknown image format: bps=" + bps + "spp=" + spp);
            return null;
        }

        int significantBits[] = new int[spp];
        for (int i = 0; i < spp; i++) significantBits[i] = bps;
    ...
    {
        // for 8 bit samples
        int bandaids[] = new int[spp];
        for (int i = 0; i < spp; i++) bandaids[i] = i;
     ...
     */
    public void setSoftMask(SoftMask m)
    {
        softmask = m;
    }
    ...
    protected static class SoftMask {
        ...
        public int getUnnormalizedAlpha(int x, int y)
        {
            ...
            switch (bitspersample) {
                case 8:
                    return alphabits[desty * rowbytes + destx] & 255;
                case 4:
    ...
    public static class AWTColorSpace extends java.awt.color.ColorSpace {
        ...
        AWTColorSpace(ColorSpace aCS)
        {
            super(aCS.getType(), aCS.getNumComponents());
            _cs = aCS;
        }

        public boolean isCS_sRGB()
        {
            return _cs.isCS_sRGB();
        }

        public float[] toRGB(float[] colorvalue)
        {
            return _cs.toRGB(colorvalue);
        }

        public float[] fromRGB(float[] rgbvalue)
        {
            return _cs.fromRGB(rgbvalue);
        }

        public float[] toCIEXYZ(float[] colorvalue)
        {
            return _cs.toCIEXYZ(colorvalue);
        }

        public float[] fromCIEXYZ(float[] colorvalue)
        {
            return _cs.fromCIEXYZ(colorvalue);
        }
        ...
        {
            return _cs.getNumComponents();
        }

        public String getName(int idx)
        {
            return _cs.getName(idx);
        }
    }
