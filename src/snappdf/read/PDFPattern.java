...
private static PDFPattern getInstance(Object pat, PDFFile srcFile)
{
    ...
    if(v instanceof Number) { int ptype = ((Number)v).intValue();
        switch(ptype) {
            case 1: return new Tiling((PDFStream)pat, srcFile);
            case 2: return Shading.getInstance(pdict, srcFile);
        }
    }
...
public static abstract class Shading extends PDFPattern implements PaintContext, Paint {
    ...
    public static Shading getInstance(Map dict, PDFFile srcFile)
    {
        ...
        switch (t) {
            case 1 : return new ShadingFunction(patternDict, shadingDict, srcFile);
            case 2 : return new ShadingAxial(patternDict, shadingDict, srcFile);
            case 3 : return new ShadingRadial(patternDict, shadingDict, srcFile);
