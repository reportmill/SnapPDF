...
public class PDFWriterBase {
    ...
    public PDFWriterBase append(char aChar)
    {
        ...
        if (aChar > 255) {
            System.err.println("PDFWriterBase: Attempt to write non-ASCII char: " + aChar);
            aChar = 'X';
        }
     ...
     */
    public PDFWriterBase append(int anInt)
    {
        return anInt < 0 ? append('-').append(-anInt) : appendDigits(anInt, 0);
    }
    ...
    public PDFWriterBase append(Rect aRect)
    {
        ...
        append((int) aRect.y);
        append(' ');
        append((int) aRect.getMaxX());
        append(' ');
        append((int) aRect.getMaxY());
        return append(']');
    }
     ...
     */
    public PDFWriterBase append(byte theBytes[])
    {
        return append(theBytes, 0, theBytes.length);
    }
    ...
    {
        _source.write(theBytes, anOffset, aLen);
        return this;
    }
     ...
     */
    public PDFWriterBase appendln(String aString)
    {
        return append(aString).appendln();
    }
    ...
    public void writePDFString(String aStr)
    {
        ...
        for (int i = 1, iMax = aStr.length() - 1; i < iMax; i++) {
    ...
    public void writeTransform(Transform aTrans)
    {
        ...
        writeTransform(m[0], m[1], m[2], m[3], m[4], m[5]);
    }
    ...
    public void writePath(Shape aShape)
    {
        ...
        double pts[] = new double[6], lx = 0, ly = 0, sx = 0, sy = 0;
        while (pathIter.hasNext()) {

            // Handle path segment types
            switch (pathIter.getNext(pts)) {
                case MoveTo:
                    moveTo(sx = lx = pts[0], sy = ly = pts[1]);
                    break;
                case LineTo:
                    lineTo(lx = pts[0], ly = pts[1]);
                    break;
                case QuadTo:
                    quadTo(lx, ly, pts[0], pts[1], lx = pts[2], ly = pts[3]);
                    break;
                case CubicTo:
                    curveTo(pts[0], pts[1], pts[2], pts[3], lx = pts[4], ly = pts[5]);
                    break;
                case Close:
                    appendln("h");
                    lx = sx;
