...
public class PDFSecurityHandler implements PDFCodec {
    ...
    public void setEncryptionParameters(byte fileID[], String ownerP, String userP, int permissionFlags)
    {
        ...
        _revision = 3;
        _access_permissions = permissionFlags;
    ...
    public byte[] getOwnerPasswordEntryBytes(String ownerP, String userP)
    {
        ...
        if (rc4Key.length > klen)
            rc4Key = copyOf(rc4Key, klen);
    ...
    public static byte[] getBytesForEncryptionEntry(Map encDict, String key)
    {
        ...
        for (int i = 1; i < slen - 1; ++i) {
            ...
            if (c == '\\') { // if it's an escape, get next char
                if (i < slen - 2) c = s.charAt(++i);
                else throw new PDFException("Illegal value in encryption dictionary");
            }
    ...
    {
        arcfour_decrypt(inout, _decrypter_key.getKeyBuffer());
    }

    /**
     * decrypt using the cached encryption key.
     */
    void arcfour_decrypt(byte inout[], byte key[])
    {
        _arcfour_decrypt(inout, key);
    }
    ...
    static void _arcfour_decrypt(byte inout[], byte key[])
    {
        ...
        for (int i = 0, j = 0, buffindex = 0, buffmax = inout.length; buffindex < buffmax; ++buffindex) {
            i = (i + 1) % 256;
            j = (j + sbox[i]) % 256;  // Generate pseudo-random K
            int temp = sbox[i];
            sbox[i] = sbox[j];
            sbox[j] = temp;
            int t = (sbox[i] + sbox[j]) % 256;
            int K = sbox[t];
            inout[buffindex] ^= K; // xor and hope for the best
        }
    }
    ...
    {
        byte[] cpy = new byte[newLen];
        System.arraycopy(orig, 0, cpy, 0, Math.min(orig.length, newLen));
        return cpy;
    }
    ...
    static class PDFBadPasswordException extends RuntimeException {
        public PDFBadPasswordException(String message)
        {
            super(message);
        }
    }
    ...
    public static void main(String args[]) throws UnsupportedEncodingException, IOException
    {
        ...
        int b;
        while ((b = System.in.read()) != -1) baos.write(b);
        byte bigbuf[] = baos.toByteArray();
        byte key[] = args[0].getBytes("US-ASCII");
        _arcfour_decrypt(bigbuf, key);
        System.out.write(bigbuf);
    }
}